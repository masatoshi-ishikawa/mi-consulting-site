...
**`src/content/articles/` に直接 `.md` ファイルを作る。ここが記事の正本。**

`_source/` を経由する必要はない。`_source/` はAstro化する前の原稿が残っているだけの下書き置き場で、`.gitignore` で除外されており**サイトからは一切参照されない**。そこに置いてもサイトには反映されない。

> **ファイルを置いただけでは公開されない。** 公開されるのは `git push` した瞬間だけ。
> 手元では `npm run dev` でいくらでも確認できる（`draft` を使う必要もない。4-1の注意を参照）。
...
### 5-2. SVG図版の後処理は自動化されている

PowerPointなどから書き出したSVGには次の問題があるが、**書き出し側の仕様で作図時には回避できない**。

1. `viewBox` が無い → 記事内で縮小表示すると図の左上しか出ない
2. `font-family` が作図ツールのフォント名のまま → サイト本文と別書体になる
3. `font-weight` が `300` → 本文（400）より細い

**これらは `scripts/fix-svg.mjs` が自動的に直す。** `npm run dev` / `npm run build` の前に自動実行されるので（`package.json` の `predev` / `prebuild`）、**手で何かする必要はない**。冪等なので、修正済みのファイルは触らない。

手で走らせたい場合:

```bash
node scripts/fix-svg.mjs
```

> フォントスタックはこのスクリプト内にも定義されている。`src/styles/global.css` の `--font-sans` を変えたら、**スクリプト側の `FONT_STACK` も直す**こと。

### 5-3. 作図時に気をつけること

**① フォントはヒラギノ角ゴシック W3**（太字は W6）。後処理で書き換わるので厳密でなくてよい。
...
**語として列挙しきれないものは、同じファイルの `KEEP_TOGETHER_PATTERNS`（正規表現）に書く。**
現在は「数字＋か月」「数字＋か月分」（`/[0-9０-９]+か月分?/`）を登録してあり、「1か／月分」のような割れ方を防ぐ。「3か月後」なら「3か月」の部分がまとまる。適用範囲は `KEEP_TOGETHER` と同じ。
